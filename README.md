...
# 4) 跑冒烟（19 项断言：新建→自动保存→重载持久→待办回写→搜索跳转→文件夹→删除等）：
SMOKE_URL=http://127.0.0.1:5174/ node web/scripts/ui-smoke.mjs
```

人工验收参考（与冒烟断言一致）：新建 → 书写 → 等待「已保存」→ 刷新页面内容仍在；
预览勾选待办后编辑器源文同步 `[x]`；搜索标题/正文关键词可命中并打开；文件夹新建/重命名/删除联动索引。
...
- 单元/集成测试：全绿（`npm test`，69+ 项，见各 `tests/*.test.mjs`）；
- `tsc --noEmit` 通过；`vite build` 无错误无告警；
- 真实 Chrome 端到端冒烟 19/19 通过（含「重启后内容仍在」验收点）。
